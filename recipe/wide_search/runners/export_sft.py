...
from recipe.wide_search.sft_export import (
    DEFAULT_ACCEPTED_STATUSES,
    WideSearchSFTExportConfig,
    export_wide_search_run_to_sft,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Export wide-search canonical traces into derived SFT datasets.")
    parser.add_argument("--run-dir", required=True, help="Run directory containing wide-search traces.")
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="If set, drop trials whose widesearch_scores sidecar score is below this value.",
    )
...
def main() -> None:
    ...
    summary = export_wide_search_run_to_sft(run_dir=Path(args.run_dir), output_path=args.output, config=config)
    print(
        json.dumps(
            {
